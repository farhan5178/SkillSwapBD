...
export function MessageBell() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      setMessages(allNotifs.filter((n: any) => n.type === "message"));
    });
  ...
  const handleMessageClick = async (message: any) => {
    // Mark as read
    if (!message.read && currentUser) {
      await updateDoc(doc(db, "users", currentUser.uid, "notifications", message.id), {
        read: true
      });
    }
  ...
  const markAllAsRead = async () => {
    if (!currentUser) return;
    const unreadMsgs = messages.filter(n => !n.read);
    
    // Process in parallel
    await Promise.all(unreadMsgs.map(n => 
                  ...
                  >
                    <div className="flex gap-3">
                      <div className={cn(
                        ...
                        !message.read ? "bg-primary" : "bg-transparent"
                      )} />
